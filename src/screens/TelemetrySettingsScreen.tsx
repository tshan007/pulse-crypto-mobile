...
const PLACEHOLDER_MEMORY_SERIES = [98, 102, 101, 108, 112, 110, 118, 124, 121, 130, 128, 136];
...
export function TelemetrySettingsScreen() {
  ...
  // Local-only for now: this does not yet call the backend to change its
  // broadcast interval. Wiring it up would mean adding a runtime-configurable
  // endpoint to the backend (currently BROADCAST_INTERVAL_MS is a startup-only
  // env var) — a deliberate follow-up, not an oversight.
  const [updateFrequency, setUpdateFrequency] = useState(250);
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      ...
      <MetricCard>
        ...
        <ThrottleSlider value={updateFrequency} onValueChange={setUpdateFrequency} />
        <Text style={styles.disclaimer}>
          Not yet wired to the backend — adjusting this changes local display only. See README.
        </Text>
        <ToggleRow
          label="Binary Protocol Compression"
          value={compressionEnabled}
          onValueChange={setCompressionEnabled}
          note="Placeholder — payloads are plain JSON today"
        />
        <ToggleRow
